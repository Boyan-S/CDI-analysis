...
def findDislocation(path, gen, rec, scannum):
    ...
    try:
        ...
        path_rec = Path(path+str(scannum)+'/results_viz/image.npy')
        rec = np.load(path_rec)
        path_supp = Path(path+str(scannum)+'/results_viz/support.npy')
        support = np.load(path_supp)
        rec = rec*support
...
def calcPRTF3D_GA(path, scannum, rec, gen):


    path_rec =  Path(path+ str(scannum)+'/results_phasing/g_%i/%i/' %(gen, rec))
    rec = np.load(path_rec / 'image.npy')
    support = np.load(path_rec /'support.npy')
    rec = rec * support
    ...
    if nRecs==None:
        ...
        for i in range(nRecs):
            try:
                ...
                if i==0:
                    path = Path(path+str(scannum)+'/results_viz/'+str(i)+'/')
...
def calcPRTF_GA(path, scannum, rec, gen, nPoints = 31, plot=False):
    ...
    PRTF_3D = calcPRTF3D_GA(path,scannum, rec, gen)
...
def CompareSlice(path, scannum, sliceNum=35, log=False, GA=False, rec=None, gen=None):
...
######
def rec_analysis(generations, cullings, num_recs, num_slice, path, scannum):
    """
    This function is trying to organize the analysis code
    """
    for gen in range(generations):
        print('Generation %i' %gen)
        for rec in range(num_recs - int(np.sum(cullings[:gen+1]))):
            print('Generation %i' %gen)
            print('Reconstruction %i' %rec)
            qPoints, PRTF, res = calcPRTF_GA(path, scannum, rec, gen, nPoints = 21, plot=True) #Phase Retrieval Transfer Function
            CompareSlice(path, scannum,sliceNum = num_slice, log=False, GA = True, rec=rec, gen=gen)
######
def rec_err_analysis(generations, cullings, num_recs, path, scannum):
    err_avg_arr = np.zeros((generations, num_recs)) * np.nan
    
    for gen in range(generations):
        for rec in range(num_recs - int(np.sum(cullings[:gen+1]))):
            path_folder = path+str(scannum)
            path_supp = path_folder +'/results_phasing/g_%i/%i/' % (gen,rec)
            err = np.load(path_supp+'errors.npy')
            err_avg_arr[gen,rec] = np.mean(err[-10:])

    plt.figure()
    plt.title('Average error of last 10 steps')
    for gen in range(generations):
        plt.plot(err_avg_arr[gen], label = 'Generation %i' %gen)
    plt.legend()
